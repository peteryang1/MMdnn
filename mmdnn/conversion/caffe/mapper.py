    ...
    def get_kernel_params(cls, node, input_shape):
        ...
        if node.kernel_parameters.global_pooling:
            ...
            if node.kind == NodeKind.Deconvolution:
                ...
                o_h_tf = ceil((input_shape.height + node.kernel_parameters.p_h * 2 - ko_h + 1) / node.kernel_parameters.s_h)
                o_w_tf = ceil((input_shape.width + node.kernel_parameters.p_w * 2 - ko_w + 1) / node.kernel_parameters.s_w)
